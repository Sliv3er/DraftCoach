...
const WinControls = () => (
  <div className="st-win-controls">
    <button className="st-win-btn st-win-min" onClick={() => ipcSend('stats-win-minimize')}>&#x2013;</button>
    <button className="st-win-btn st-win-close" onClick={() => ipcSend('stats-win-close')}>&times;</button>
  </div>
);
...
export function StatsWindow() {
  ...
  useEffect(() => {
    ipcOn('stats-data', (_e: any, d: StatsData) => { setStats(d); setLoading(false); setError(''); });
    ipcOn('stats-error', (_e: any, msg: string) => { setError(msg); setLoading(false); });
    ipcOn('stats-log', (_e: any, msg: string) => setDebugLog(p => [...p.slice(-50), msg]));
    return () => { ipcRemoveListener('stats-data'); ipcRemoveListener('stats-error'); ipcRemoveListener('stats-log'); };
  }, []);

  useEffect(() => { if (debugRef.current) debugRef.current.scrollTop = debugRef.current.scrollHeight; }, [debugLog]);

  const fetchStats = () => { setLoading(true); setError(''); ipcInvoke('fetch-my-stats').catch((e: any) => { setError(e.message); setLoading(false); }); };
  ...
  const analyzeGame = async (idx: number) => {
    ...
    try {
      const raw = await ipcInvoke('analyze-single-game', idx);
  ...
  const viewPlayerProfile = async (name: string) => {
    ...
    try {
      ...
      const data = await ipcInvoke('fetch-player-stats', pName, pTag);
