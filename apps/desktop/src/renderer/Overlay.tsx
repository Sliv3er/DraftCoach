...
export function Overlay() {
    ...
    useEffect(() => {
        ...
        const fetchAuto = async () => {
            const detected = await ipcRenderer.invoke('get-autodetect-hud');
    ...
    useEffect(() => {
        ...
        ipcRenderer.on('overlay-items-update', itemHandler);
        ...
        ipcRenderer.on('cooldown-tick', cdHandler);
        ...
        ipcRenderer.on('scoreboard-data', sbHandler);

        return () => {
            ipcRenderer.removeListener('overlay-data-update', handler);
            ipcRenderer.removeListener('settings-update', settingsHandler);
            ipcRenderer.removeListener('overlay-items-update', itemHandler);
            ipcRenderer.removeListener('cooldown-tick', cdHandler);
            ipcRenderer.removeListener('scoreboard-data', sbHandler);
        };
    }, []);
    ...
    useEffect(() => {
        ...
        ipcRenderer.on('item-purchase-update', handler);
        return () => { ipcRenderer.removeListener('item-purchase-update', handler); };
    }, [data]);
...
function EnemySpellPanel({ enemies, timers, isOpen, onToggle }: EnemySpellPanelProps) {
    ...
    const handleMouseEnter = () => {
        ipcRenderer.send('overlay-set-ignore-mouse', false);
    };
    ...
    const handleClick = (championName: string, ability: string) => {
        ...
        if (existing && existing.remaining > 0) {
            ipcRenderer.invoke('cooldown-reset', { timerId });
        } else {
            ipcRenderer.invoke('cooldown-start', { championName, ability });
        }
    };
