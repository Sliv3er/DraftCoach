...
export function ScoreboardWindow() {
  ...
  useEffect(() => {
    ...
    ipcRenderer.on('scoreboard-data', dataHandler);

    // Listen for cooldown timer ticks
    const timerHandler = (_e: any, activeTimers: CooldownTimer[]) => {
      setTimers(activeTimers);
    };
    ipcRenderer.on('cooldown-tick', timerHandler);

    // Window controls
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        ipcRenderer.send('scoreboard-win-hide');
      }
    };
    window.addEventListener('keydown', handleKeyDown);

    return () => {
      ipcRenderer.removeListener('scoreboard-data', dataHandler);
      ipcRenderer.removeListener('cooldown-tick', timerHandler);
