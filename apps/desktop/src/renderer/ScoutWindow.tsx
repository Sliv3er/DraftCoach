...
export function ScoutWindow() {
  ...
  useEffect(() => {
    ...
    ipcRenderer.on('scout-report', (_e: any, d: ScoutReport) => setReport(d));
    ipcRenderer.on('scout-status', (_e: any, d: any) => setStatus(d));
    ipcRenderer.on('scout-debug', (_e: any, l: string) => setDebugLog(p => [...p.slice(-80), l]));
    ipcRenderer.invoke('scout-get-cached').then((c: ScoutReport | null) => {
      if (c) { setReport(c); setStatus({ phase: 'done', message: '' }); }
    });
    return () => { ipcRenderer.removeAllListeners('scout-report'); ipcRenderer.removeAllListeners('scout-status'); ipcRenderer.removeAllListeners('scout-debug'); };
  }, []);
