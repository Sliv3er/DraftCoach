...
const SCENARIOS = [
  ...
  { name: 'Darius top confirmed Kayle lane roles', myChampion: 'Darius', role: 'top', allies: ['Vladimir', 'Graves', 'Bard', 'Yone'], enemies: ['Senna', 'Lucian', 'Vi', 'Galio', 'Kayle'], enemyRoles: { Senna: 'adc', Lucian: 'mid', Vi: 'jungle', Galio: 'support', Kayle: 'top' }, expect: { boots: ['Plated Steelcaps'], noText: ['Senna Top'], mustText: ['Kayle'] } },
...
function runStaticAdvisorChecks() {
  ...
  if (!source.includes('lcu-role-snapshot') || !source.includes('gameflow-role-snapshot') || !source.includes('live-client-position')) issues.push('loading/live confirmed role snapshot endpoint is missing');
  if (!source.includes('classifyLiveEnemyDamage') || !source.includes('LIVE ITEMIZATION OVERRIDES') || !source.includes('lastEnemyDamageTypes')) issues.push('live advisor cannot adapt when enemy itemization changes damage type');
